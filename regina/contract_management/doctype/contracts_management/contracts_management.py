...
class ContractsManagement(Document):
    ...
    def validate_items_values(self) :
        for item in self.items :
            if float(item.total_cash or 0) + float(item.total_payment_papers or 0) != float(item.total_amount or 0) and not None :
                ...
                frappe.throw(_(f""" Total Amount is {item.total_amount}
                             and total cash is {item.total_cash} 
                                    ...
                                    Please validate amounts
                                      difference is {float(total_payments) - float(item.total_amount)} """))  
    ...
    def create_payment_paper(self):
        for item in self.items:
            if item.receive_date and item.contract_serial_number and \
                    item.total_payment_papers_count is not None and \
                    item.total_payment_papers is not None:
